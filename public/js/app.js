...
$.getJSON("http://api.flickr.com/services/feeds/photos_public.gne?id="+id+"&lang=it-it&format=json&jsoncallback=?", function(data){
        $.each(data.items, function(i,item){
            $("<img class='imgflikr' />")
            .attr("src", item.media.m).appendTo("#app")
            .wrap("<div class='imagesflickr'><a title='"+item.tags+"' rel='prettyPhoto[flickrgallery]' href='"+item.media.m.replace("_m.jpg", "_z.jpg")+"'></a></div>")
            .mouseover(function() {
